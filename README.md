...
### Logo real, título "Empresa" y catálogo de productos con importador por IA

- **Logo de Microvidas en el PDF y en Cotizaciones:** el PDF de cotización ahora usa el isologo real de Microvidas (en vez del texto simulado "MICROVIDAS / AGROBIOTECNOLOGÍA"), y el mismo logo aparece también en el encabezado de la pantalla de Cotizaciones.
- **Título "Empresa":** tanto en el PDF como en la pantalla (al crear/editar una cotización y en el detalle), ahora se destaca de forma prominente a qué empresa se le está cotizando (por ejemplo "Empresa: Agroindustrias del Valle SA"), en vez de la línea chica "Para:" que había antes.
- **Catálogo de productos (nueva sección "Catálogo" en el menú):** antes los productos sólo se podían cargar por la base de datos; ahora hay una pantalla propia para ver, crear, editar y desactivar/reactivar productos del catálogo que se usa en Cotizaciones y Ventas.
- **Proveedores con logo:** los productos se pueden agrupar por proveedor (marca de origen), cada uno con su propio logo. Los proveedores se pueden editar (nombre/logo) o borrar (sólo si no tienen productos activos) desde la misma pantalla.
- **Importar lista de precios en PDF con IA:** el botón "Importar lista de precios (PDF)" del Catálogo permite subir el PDF de la lista de precios de un proveedor; una IA (Claude, de Anthropic) lee el documento y devuelve el nombre del proveedor, la moneda y cada producto con su precio de lista, y el sistema intenta además detectar automáticamente el logo del proveedor dentro del propio PDF. Todo esto se muestra en una vista previa editable (se puede corregir cualquier producto, agregar o quitar filas, y reemplazar el logo detectado subiendo otro) antes de guardar definitivamente en el catálogo — nada se guarda sin confirmar. Los productos importados quedan como cualquier otro producto del catálogo: se pueden editar o desactivar después sin restricciones. Requiere configurar una clave de IA en el servidor (`ANTHROPIC_API_KEY`, ver `DEPLOY.md`); sin esa clave el resto del CRM sigue funcionando igual y sólo ese botón muestra un aviso.
